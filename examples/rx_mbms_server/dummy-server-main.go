...
func main() {
	...
	if err := diam.ListenAndServe(*addr, mux, nil); err != nil {
		log.Fatal(err)
	}
}
...
func printErrors(ec <-chan *diam.ErrorReport) {
	for err := range ec {
		log.Printf("diameter error: %v", err)
	}
}
...
func handleRxAAR(settings *sm.Settings, verbose bool) diam.HandlerFunc {
	return func(c diam.Conn, m *diam.Message) {
		...
		logInterestingAVPs("RX", m, []interface{}{avp.AFApplicationIdentifier, avp.MediaComponentDescription, avp.RxRequestType, avp.SubscriptionID})
		if verbose {
			log.Printf("RX: incoming AAR\n%s", m)
		}
...
func firstTMGI(m *diam.Message) datatype.OctetString {
	if a, err := m.FindAVP(avp.TMGI, diam.TGPP_VENDOR_ID); err == nil {
		if v, ok := a.Data.(datatype.OctetString); ok && v != "" {
			return v
		}
	}
	return defaultTMGI
...
func addServerOrigin(m *diam.Message, settings *sm.Settings) {
	m.NewAVP(avp.OriginHost, avp.Mbit, 0, settings.OriginHost)
	m.NewAVP(avp.OriginRealm, avp.Mbit, 0, settings.OriginRealm)
}
func copyAVP(dst, src *diam.Message, code interface{}, vendorID uint32) {
	if a, err := src.FindAVP(code, vendorID); err == nil && a != nil {
		dst.AddAVP(a)
	}
}
func avpString(m *diam.Message, code interface{}) string {
	if a, err := m.FindAVP(code, 0); err == nil && a != nil && a.Data != nil {
...
func logInterestingAVPs(prefix string, m *diam.Message, codes []interface{}) {
	...
	for _, code := range codes {
		...
		if err != nil || len(avps) == 0 {
			avps, err = m.FindAVPs(code, 0)
		}
		if err == nil && len(avps) > 0 {
			parts = append(parts, fmt.Sprint(code)+"="+avps[0].Data.String())
		}
	}
	if len(parts) > 0 {
		log.Printf("%s: key AVPs: %s", prefix, strings.Join(parts, ", "))
	}
}

func writeAnswer(prefix string, c diam.Conn, a *diam.Message, verbose bool) {
	if verbose {
		log.Printf("%s: outgoing answer\n%s", prefix, a)
	}
	if _, err := a.WriteTo(c); err != nil {
		log.Printf("%s: failed to write answer to %s: %v", prefix, c.RemoteAddr(), err)
	}
}
func handleAll(c diam.Conn, m *diam.Message) {
	log.Printf("received unhandled Diameter message from %s cmd=%d app=%d flags=0x%x\n%s", c.RemoteAddr(), m.Header.CommandCode, m.Header.ApplicationID, m.Header.CommandFlags, m)
}
